...
| `Agent Dispatch Guide` | `/gvt-dev:plan-task` (Phase 1) | The project's domain-specific recon/explorer agent(s) (e.g. `gvt-construct3:c3-explorer`) so planning dispatches domain-aware recon instead of the generic `gvt-dev:analyst`. Omit it and planning falls back to the analyst. |
